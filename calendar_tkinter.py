...
# lbl = Label(window, text="Календарь", font=("Arial Bold", 50))
# lbl.grid(column=0, row=0)
# btn = Button(window, text="next", command=next)
# btn.grid(column=1, row=0)


# функция логики отрисовки
def fill():
    ...
    for n in range(month_days):
        ...
        days[n + week_day]['fg'] = 'snow'
        if year == now.year and month == now.month and n == now.day - 1:
            # days[n]['background'] = 'OrangeRed2'
            days[week_day + n]['background'] = 'OrangeRed2'
        else:
            days[n + week_day]['background'] = 'gray25'
    for n in range(week_day):
        days[week_day - n - 1]['text'] = prew_month_days - n
        days[week_day - n - 1]['fg'] = 'gray'
        days[week_day - n - 1]['background'] = '#000000'
...
info_label = Label(root, text='0', width=1, height=1,
                   font=('terminal', 16, 'bold'), fg='black')
